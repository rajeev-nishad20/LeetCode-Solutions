class Solution {
    ...
    private int solve(int[] walls, int[][] roboDist, int[][] range, int i, int preDir){
        ...
        return t[i][preDir] = Math.max(leftTake, rightTake);
    ...
    public int maxWalls(int[] robots, int[] distance, int[] walls) {
        ...
        for(int i = 0; i < n; i++){
            ...
            int lLimit = (i == 0) ? 1 : roboDist[i - 1][0] + 1;
            int rLimit = (i == n - 1) ? (int)1e9 : roboDist[i + 1][0] - 1;

            int L = Math.max(pos - d, lLimit);
            int R = Math.min(pos + d, rLimit);
